...
async fn main() {
    ...
    let tracer = global::tracer("z_put.rs");
    let span = tracer
        .span_builder("Start and put the data to subscriber")
        .with_attributes(vec![
            trace::MESSAGING_SYSTEM.string("zenoh"),
            trace::MESSAGING_OPERATION.string("send"),
            trace::MESSAGING_DESTINATION.string("z_sub")
        ])
        .start(&tracer);
    ...
    println!("New zenoh...");
    cx.span().add_event("New zenoh client API".into(), vec![]);
    let zenoh = Zenoh::new(config.into()).with_context(cx.clone()).await.unwrap();

    println!("New workspace...");
    cx.span().add_event("New zenoh workspace.".into(), vec![]);
    let workspace = zenoh.workspace(None).with_context(cx.clone()).await.unwrap();

    println!("Put Data ('{}': '{}')...\n", path, injector["traceparent"]);
    let span_context = injector["traceparent"].clone();
    cx.span().add_event("Put the span context".into(), vec![KeyValue::new("span context", span_context.clone())]);
    workspace
        .put(&path.try_into().unwrap(), span_context.into())
